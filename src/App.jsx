...
export default function App() {
  ...
  const handleAddTrip = (newTripData) => {
    ...
    posthog?.capture('trip_created', {
      ...
      trip_start_date: newTrip.startDate,
      trip_end_date: newTrip.endDate,
    });
  };
  ...
  const openTimelineForTrip = (tripId) => {
    ...
    posthog?.capture('trip_timeline_opened', {
      trip_id: tripId,
    });
  };
  ...
  const handleAddEvent = (newEventData) => {
    ...
    posthog?.capture('trip_event_added', {
        ...
        {trips.length === 0 ? (
          ...
          trips.map((trip) => (
            ...
            >
              <h3>{trip.title}</h3>
              <p>
                Дати: {trip.startDate} - {trip.endDate}
              </p>
            </div>
          ))
        )}
  ...
  return (
    <div id="timeline-screen">
      <header className="p-4">
        <button onClick={goBack} className="text-blue-500 mb-2 block">
          ← Назад
        </button>
        <div className="flex justify-between items-center">
          <h2 id="current-trip-title" className="text-xl font-bold">
            {trip.title}
          </h2>
          <button className="btn-secondary" onClick={openEventModal}>
            + Додати подію
          </button>
        </div>
      </header>
      <main id="timeline-container" className="timeline-line p-4">
        {events.length === 0 ? (
          ...
          events.map((event) => (
            <div
              key={event.id}
              className={`event-card event-type-${event.type} border-l-4 p-4 my-2`}
            >
              ...
              {event.notes && (
                <p className="notes text-gray-600 text-sm mt-1">
                  {event.notes}
                </p>
              )}
            </div>
  ...
  return (
    <div className="modal-backdrop" onClick={closeModal}>
      ...
      >
        ...
        <textarea
          name="notes"
          placeholder="Бронювання, адреси тощо..."
        ></textarea>
