...
describe('Тестування бізнес-логіки Multi-City Timeline', () => {
  ...
  const mockEvents = [
    { id: 2, title: 'Чек-ін у готель (Відень)', date: '2026-05-23', time: '14:00', type: 'Житло' },
    { id: 1, title: 'Поїзд Краків - Відень', date: '2026-05-23', time: '08:30', type: 'Транспорт' },
    { id: 3, title: 'Оренда велосипедів (Загреб)', date: '2026-05-25', time: '13:00', type: 'Активності' }
  ];
  ...
  test('2. validateEvent має повертати true для коректної картки події', () => {
    const validEvent = { title: 'Плітвіцькі озера', date: '2026-05-24', time: '09:00' };
  ...
  test('3. validateEvent має викидати помилку, якщо пропущено назву або дату', () => {
    ...
    expect(() => validateEvent(invalidEvent)).toThrow("Відсутні обов'язкові поля для події");
  });
  ...
  test('4. validateEvent має викидати помилку, якщо час завершення менший за час початку', () => {
    ...
    expect(() => validateEvent(brokenTimeEvent)).toThrow("Час завершення не може бути ранішим за час початку");
  });
